    ...
    if (rover) {
      fetch(
        ...
        .then((data) => {
          ...
          const dataR = data.photo_manifest

          function Rovercheck() {
            if (roverdata.length === 0) {
              setRoverData(dataR)
            }
            if (roverdata?.some((e) => e.name === rover)) {
            } else setRoverData([...roverdata, dataR])
          }
          Rovercheck()
          console.log(roverdata)

          function Roversearch() {
            if (roverdata.some((r) => r.name === rover)) {
              setSolOptions(
                roverdata
                  ?.filter((r) => r.name === rover)[0]
                  .photos.map((photo) => photo.sol)
              )
